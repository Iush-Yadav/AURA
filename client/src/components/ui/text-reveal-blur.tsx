...
const WIPE_SPEED_PX_PER_SEC = 400;
const REVEAL_SPEED_PX_PER_SEC = 420;
const MIN_WIPE_DURATION = 0.8;
const MAX_WIPE_DURATION = 1.4;
const MIN_REVEAL_DURATION = 0.8;
const MAX_REVEAL_DURATION = 1.6;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {currentWord.split(" ").map((word, wIdx, arr) => {
            const startIndex =
              currentWord.split(" ").slice(0, wIdx).join(" ").length +
              (wIdx > 0 ? 1 : 0);
            return (
                  ...
                  {word.split("").map((char, cIdx) => {
                    ...
                    return (
                      <span
                        key={globalIdx}
                        ref={(node) => {
                          charsRef.current[globalIdx] = node;
                        }}
                        className="char"
                        style={{
                          display: "inline-block",
...
export default function TextRevealBlur(props: Record<string, unknown>) {
